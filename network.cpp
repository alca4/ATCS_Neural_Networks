...
struct NeuralNetwork 
{
   ...
   */
   ...
   unordered_map<string, int> PARAMETERS = {{"numActivationLayers", 0},
                                            ...
                                            {"earlyStoppingCycles", 16},
                                            {"earlyStoppingThreshold", 17},
                                            {"beta1", 18},
                                            {"beta2", 19},
                                            {"numInEachClass", 20},};
      ...
      while (!fin.eof())
      {
         ...
         switch (val)
         {
            ...
            case 16:    // earlyStoppingCycles
               {
                  earlyStoppingCycles = getNextInteger(fin);
                  if (numTests <= 0)
                  {
            ...
            case 17:    // earlyStoppingThreshold
               {
                  earlyStoppingThreshold = getNextDouble(fin);
                  if (numTests <= 0)
                  {
   ...
   */
   void forwardPassTrain(int testCase) 
   {
      ...
      {
         theta[a][b] = 0.0;
         for (int c = 0; c < activationLayerSize[lastHiddenLayer]; c++)
            theta[a][b] += activations[lastHiddenLayer][c] * weights[lastHiddenLayer][c][b];

         activations[a][b] = thresholdFunction(theta[a][b]);

         psi[a][b] = (trueValues[testCase][b] - activations[a][b]) *
                      thresholdFunctionDerivative(theta[a][b]);
      } // for (int b = 0; b < activationLayerSize[outputLayer]; b++)
   ...
   void train()
   {
      ...
      do 
      {
         ...
         if (avgError + earlyStoppingThreshold - lastCycleError > 0) lastImprovementCycle++;
            ...
            lastCycleError = avgError;
         }
      } // do
      while (iterations < maxIterations && avgError - errorThreshold > EPSILON && lastImprovementCycle < earlyStoppingCycles);
      ...
      else if (lastImprovementCycle == earlyStoppingCycles)
         cout << "Model terminated due to early stopping." << endl;
      else 
